...
public class CultureDiServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String ccbaCtcd = req.getParameter("CcbaCtcd");
		String ccbaAsno = req.getParameter("CcbaAsno");
		int ccbaKdcd = Integer.parseInt(req.getParameter("CcbaKdcd")); 
		
		System.out.println(ccbaMnm1);
		System.out.println(ccbaAsno);
		System.out.println(ccbaKdcd);
		System.out.println(ccbaCtcd);
		
		CultureDiVo culture = service.getcultCultureDiVo(ccbaMnm1);
 		List<CultureImgVo> imagelist = service.getCultureImgVos(ccbaAsno, ccbaCtcd, ccbaKdcd);
